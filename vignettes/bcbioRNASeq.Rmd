...
```{r}
salmon_genes <- bcbioRNASeq(uploadDir = uploadDir, caller = "salmon")
assayNames(salmon_genes)
```

```{r}
salmon_tx <- bcbioRNASeq(uploadDir = uploadDir, level = "transcripts")
...
```{r}
star <- bcbioRNASeq(uploadDir = uploadDir, caller = "star")
assayNames(star)
```
...
```{r}
# `export()` invisibly returns a character vector containing file paths.
