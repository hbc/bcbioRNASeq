...
[bcbioRNASeq][] integrates with [DESeq2][], which we recommend for differential expression. To prepare our dataset for differential expression, we need to coerce the `bcbioRNASeq` object to a `DESeqDataSet`. Use the `as()` function.
