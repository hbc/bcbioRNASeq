...
.plotPCA.DESeqTransform <- function(  # nolint
    ...
    if (!(is.character(genes) || is.null(genes))) {
        abort("`genes` must be a character vector or NULL")
    }
    # color
    if (!(is(color, "ScaleDiscrete") || is.null(color))) {
        abort("`color` must be ggproto ScaleDiscrete or NULL")
    }
    # label
    if (!is.logical(label)) {
        abort("`label` must be logical")
    }
    # returnData
    if (!is.logical(returnData)) {
        abort("`returnData` must be logical")
    }
