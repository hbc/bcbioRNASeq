...
#' (`groups_of_interest`) specified in the [bcbioRnaDataSet] and then the sample names.
#'
...
#' @param groups_of_interest Interesting groups to label with bars above heatmap. If
#'   `NULL`, defaults to `groups_of_interest` defined in [bcbioRnaDataSet].
...
plot_correlation_heatmap <- function(
    ...
    if (is.null(groups_of_interest)) {
        groups_of_interest <- run$groups_of_interest
    }

    # Obtain the metadata annotations, if not specified
    if (is.null(annotation)) {
        annotation <- select_groups_of_interest_coldata(dt, groups_of_interest)
    }
...
plot_gene_heatmap <- function(
    ...
    # If `cluster_cols = FALSE`, sort by `groups_of_interest` then `description`
    if (!isTRUE(cluster_cols)) {
        sorted_cols <- colData(dt) %>%
            as.data.frame %>%
            arrange(!!!syms(c(groups_of_interest, "description"))) %>%
            pull("description")
