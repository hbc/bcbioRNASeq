...
#' @param groups_of_interest Interesting groups to use for point appearance. If `NULL`,
#'   color defaults to all `groups_of_interest` parameters set in [bcbioRnaDataSet].
...
plot_pca <- function(
    ...
    groups_of_interest_name <- paste(groups_of_interest, collapse = " :\n")

    # `group` column is generated by `plotPCA()` using `groups_of_interest`
    data <- plotPCA(dt,
    ...
    # Always define color by `groups_of_interest`
    ...
    plot <- ggplot(
        ...
        labs(title = paste("pca", name, sep = label_sep),
             ...
             color = groups_of_interest_name,
             shape = groups_of_interest_name)
