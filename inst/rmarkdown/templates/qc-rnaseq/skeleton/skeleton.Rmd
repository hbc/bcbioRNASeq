...
Here we're seeing the nuclei samples vary the most relative to the others.

```{r plot_pca, fig.height=12, fig.width=10}
plot_pca(bcb)
```

Let's take a look at how the other samples are clustering with the nuclei samples removed. Now we can see that the samples are clustering by treatment, with the D9 samples strongly separated on PC1 from the D17 samples. This is good to observe. Note that there may be something distinct about the e_d17_enuclate replicate, which seems to differ from the other biological replicates (green). Was there something different in how this sample was prepared?

```{r plot_pca_censor}
plot_pca(bcb,
         interesting_groups = "treatment",
         censor_samples = c("c_d17_nuclei",
                            "d_d17_nuclei",
                            "e_d17_nuclei"))
```
...
plot_pca_covariates(bcb, metrics = FALSE)
```
