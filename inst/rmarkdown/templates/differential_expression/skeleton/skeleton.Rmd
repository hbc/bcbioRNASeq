...
# Render quality control template first to save bcb (bcbioRNADataSet)
source("setup.R")
library(DESeq2)
...
Let's take a look at the number of genes we get with different false discovery
rate (FDR) cutoffs. These tests subset *P* values that have been multiple test
corrected using the Benjamini Hochberg (BH) method [@Benjamini:1995ws].
...
We performed the analysis using a BH adjusted *P* value cutoff of `r alpha` and
a log fold-change (LFC) ratio cutoff of `r lfc`.
...
An MA plot compares transformed counts on `M` (log ratio) and `A` (mean average)
scales [@Yang:2002ty].
...
A volcano plot compares significance (BH-adjusted *P* value) against fold change
(log2) [@Cui:2003kh; @Li:2014fv]. Genes in the green box with text labels have
an adjusted *P* value are likely to be the top candidate genes of interest.
...
have scaled the data by row and used the `ward.D2` method for clustering
[@WardJr:1963eu].
...
The results are saved as gzip-compressed comma separated values (CSV). Gzip
compression is natively supported on [macOS][] and Linux-based operating
systems. If you're running Windows, we recommend installing [7-Zip][]. CSV files
can be opened in [Excel][] or [RStudio][].
...
- `padj`: BH adjusted Wald test *P* value (corrected for multiple comparisons;
    aka FDR).
...
Only the top up- and down-regulated genes (arranged by log2 fold change) are
shown.
