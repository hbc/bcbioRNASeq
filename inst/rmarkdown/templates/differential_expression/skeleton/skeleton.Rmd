...
res <- res_shrunken
save_data(res, res_unshrunken, res_shrunken)
```
...
```{r plot_volcano}
plot_volcano(run, res = res, lfc = lfc)
```
...
```{r plot_deg_heatmap}
plot_deg_heatmap(run, res = res, dt = rld, lfc = lfc)
```
...
res_tbl <- res_tables(run, res = res, lfc = lfc)
save_data(res_tbl)
```
