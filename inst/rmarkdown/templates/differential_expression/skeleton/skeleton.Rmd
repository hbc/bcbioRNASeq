...
We performed the analysis using a BH adjusted *P* value cutoff of `r params$alpha` and a log fold-change (LFC) ratio cutoff of `r params$lfcThreshold`.
...
```{r plot_volcano}
plotVolcano(res, lfcThreshold = params$lfcThreshold)
```
...
res_tbl <- resultsTables(
    results = res,
    counts = dds,
    lfcThreshold = params$lfcThreshold,
    write = TRUE,
    summary = TRUE,
    headerLevel = 2,
