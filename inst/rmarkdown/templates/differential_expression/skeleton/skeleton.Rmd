...
# Also refer to "Hypothesis tests with thresholds on effect size" section in
# the DESeq2 paper for more explanation on how to set `results()`.
# Don't use post hoc LFC threshold filtering. 
...
Now let's calculate shrunken log2 fold change values with `DESeq2::lfcShrink()`. We're using the "normal" shrinkage estimator (default in DESeq2); the "apeglm" shrinkage estimator is also promising but doesn't work well with complex contrast designs.
