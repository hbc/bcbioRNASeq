...
params:
    dds_file: "data/YYYY-MM-DD/dds.rda"
...
# This will be stashed when using `as(dds, "bcbioRNASeq")` coercion method.
...
res_subset <- res[!is.na(res$padj), , drop = FALSE]
all_genes <- rownames(res_subset)

res_df <- as.data.frame(res_subset)
...
dds_subset <- dds[all_genes, ]
entrez <- rowData(dds_subset)$entrezID
...
# Coerce Entrez identifiers to character
