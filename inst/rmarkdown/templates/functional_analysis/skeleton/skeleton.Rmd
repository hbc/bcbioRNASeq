...
# KEGG supported organismss
# http://www.genome.jp/kegg/catalog/org_list.html

# OrgDb supported organisms
# http://bioconductor.org/packages/release/BiocViews.html#___OrgDb

# OrgDb package (e.g. "org.Hs.eg.db")
library(stringr)
match <- str_match(
...
entrez_res_df$entrezID <- gene2entrez
...
# Significant results by Entrez ID
sig_entrez_res_df <- entrez_res_df %>%
    .[sig_genes, , drop = FALSE] %>%
    rownames_to_column("geneID") %>%
    select(entrezID, everything()) %>%
    group_by(entrezID) %>%
    # Get the most significant result, per Entrez ID
    top_n(n = 1, wt = padj)
stopifnot(!any(duplicated(sig_entrez_res_df$entrezID)))
