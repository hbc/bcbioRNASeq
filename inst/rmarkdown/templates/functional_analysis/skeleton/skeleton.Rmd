...
resultsDir <- file.path(params$outputDir, "results", "functional_analysis")
keggPlotsDir <- file.path(resultsDir, "kegg_plots")
invisible(mapply(
    FUN = dir.create,
    list(dataDir, resultsDir, keggPlotsDir),
...
write_csv(
    egoSummary,
    path = file.path(resultsDir, paste0(paste(
        "go",
...
write_csv(
    gseaKEGGSummary,
    path = file.path(resultsDir, "gsea_kegg_clusterprofiler.csv.gz"))
gseaKEGGSummary
```
