...
results_dir <- file.path(params$output_dir, "results", "functional_analysis")
kegg_plots_dir <- file.path(results_dir, "kegg_plots")
invisible(mapply(
    FUN = dir.create,
    list(data_dir, results_dir, kegg_plots_dir),
...
enrich_go <- enrichGO(
    ...
    qvalueCutoff = 0.05,
    readable = TRUE)
saveData(enrich_go, dir = data_dir)
...
write_csv(
    enrich_go_summary,
    path = file.path(results_dir, paste0(paste(
        "go",
...
gsea_kegg <- gseKEGG(
    ...
    pvalueCutoff = 0.05,
    verbose = FALSE)
saveData(gsea_kegg, dir = data_dir)

# Extract the GSEA results
gsea_kegg_summary <- slot(gsea_kegg, "result") %>% as_tibble()
write_csv(
    gsea_kegg_summary,
    path = file.path(results_dir, "gsea_kegg_clusterprofiler.csv.gz"))
gsea_kegg_summary
```
