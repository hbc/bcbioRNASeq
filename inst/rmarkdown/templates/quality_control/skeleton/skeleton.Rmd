...
```{r sample_data}
sample_data <- sampleData(bcb, clean = TRUE)
export(
    x = sample_data,
...
fpkm <- fpkm(as(bcb, "DESeqDataSet"))
saveData(raw_counts, normalized_counts, tpm, fpkm, dir = params$data_dir)
writeCounts(raw_counts, normalized_counts, tpm, fpkm, dir = params$results_dir)
```
...
- [`fpkm.csv.gz`](`r file.path(params$results_dir, "fpkm.csv.gz")`): Fragments per kilobase per million mapped fragments. Legacy format. We recommend using transcripts per million instead.
...
if ("rrnaRate" %in% colnames(metrics(bcb))) {
    plotRRNAMappingRate(bcb)
} else {
    message("rRNA mapping rate was not calculated.")
}
```
...
if ("geneBiotype" %in% colnames(rowData(bcb))) {
    plotCountsPerBiotype(bcb, normalized = "tpm")
} else {
    message("Biotypes are not defined in `rowData()`.")
}
```
...
if ("broadClass" %in% colnames(rowData(bcb))) {
    plotCountsPerBroadClass(bcb, normalized = "tpm")
} else {
    message("Broad class biotypes are not defined in `rowData()`.")
}
```

## Count counts per gene
...
```{r identical_samples, echo=FALSE, message=FALSE, results="hide"}
...
if (isTRUE(identical_samples)) {
    message("Samples are identical. Skipping dispersion plot.")
} else {
    plotDispEsts(bcb)
}
```
...
```{r plot_pca}
if (isTRUE(identical_samples)) {
    message("Samples are identical. Skipping PCA.")
} else {
    plotPCA(
        object = bcb,
        normalized = "vst",
...
if (isTRUE(identical_samples)) {
    message("Samples are identical. Skipping correlation heatmap.")
} else {
