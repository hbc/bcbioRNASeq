---
# Alternatively, consider using our DESeqAnalysis package for complex contrasts.
#
# Example bcbioRNASeq object to check the template (from vignette):
#  
...
Ensure that all relevant factor columns in `sampleData()` contain valid names (see `make.names()` for details) prior to setting the design. Here we are using the `snake()` function to automatically sanitize all factors into snake case.

```{r sanitize-factors}
sd <- sampleData(dds)
rn <- rownames(sd)
sd <- lapply(X = sd, FUN = snake)
sd <- as(sd, "DataFrame")
rownames(sd) <- rn
sampleData(dds) <- sd
```
